...
class GLViewer:
    ...
    def init(self, _params, _mesh, _create_mesh): 
        ...
        glutCreateWindow(b"PACMAN Spatial Mapping")
    ...
    def draw(self):  
        if self.available:
            # Skip drawing background image - it's shown in separate OpenCV window
            # self.image_handler.draw()
            
            # Clear to black background for mesh-only view
            glClearColor(0.0, 0.0, 0.0, 1.0)
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)

            # If the Positional tracking is good, we can draw the mesh
